...
const getAllNonApprovedWorkEntries = async (req, res) => {
  try {
    ...
    console.error(
      "Error fetching non-approved work data:",
      error.message || error
    );
    res.status(500).json({
      error:
        "Failed to fetch non-approved work entries. Please try again later.",
    });
  }
};
...
const addWork = async (req, res) => {
  ...
  try {
    ...
    await db.query("SET timezone = 'Europe/Istanbul';");
...
const deleteWorkEntry = async (req, res) => {
  ...
  try {
    const result = await db.query(
      "DELETE FROM other_works WHERE id = $1 RETURNING *",
      [id]
    );
    ...
    res
      .status(200)
      .json({ success: true, message: "Work entry deleted successfully" });
  } catch (error) {
...
const editWorkEntry = async (req, res) => {
  const { id } = req.params;
  const { work_type, date, day, time, workload, is_approved } = req.body;

  if (!id || !work_type || !date || !day || !time || !workload) {
    return res
      .status(400)
      .json({ error: "All fields are required for editing the work entry." });
  }
...
const updateWork = async (req, res) => {
  ...
  try {
    if (work_type === "Tour") {
      ...
      return res
        .status(200)
        .json({ success: true, message: "Tour entry updated successfully." });
      ...
      return res.status(200).json({
        success: true,
        message: "Individual Tour entry updated successfully.",
      });
      ...
      return res
        .status(200)
        .json({ success: true, message: "Fair entry updated successfully." });
    } else {
      await db.query(
      ...
      return res.status(200).json({
        success: true,
        message: "Other work entry updated successfully.",
      });
    }
  } catch (error) {
...
const saveWorkload = async (req, res) => {
  ...
  if (workload == null || workload <= 0) {
    return res
      .status(400)
      .json({ success: false, message: "Invalid workload value." });
  }
  ...
  try {
    ...
    if (result.rowCount === 0) {
      return res
        .status(404)
        .json({ success: false, message: "Work entry not found." });
    }

    res.status(200).json({
      success: true,
      message: `Workload updated successfully for ${workType}.`,
    });
  } catch (error) {
