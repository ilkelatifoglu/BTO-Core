...
const Sidebar = ({ setCurrentPage }) => {
    ...
    const [profileImage, setProfileImage] = useState(cachedProfileImage || defaultProfileImage);
    const [userName, setUserName] = useState(cachedUserProfile ? `${cachedUserProfile.firstName} ${cachedUserProfile.lastName}` : '');
    ...
    useEffect(() => {
        if (!cachedProfileImage || !cachedUserProfile) {
            ...
            setUserName(`${cachedUserProfile.firstName} ${cachedUserProfile.lastName}`);
        }
    ...
    return (
        <div style={{ display: "flex" }}>
            <div className={`sidebar ${isExpanded ? 'expanded' : 'collapsed'}`}>
                ...
                <button className="sidebar__toggle" onClick={toggleSidebar} style={{marginRight: '15px'}}>
                    {isExpanded ? '<<' : '>>'}
                </button>
